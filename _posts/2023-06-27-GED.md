...
本文是从报告中导出的，一些公式之类的东西可能有还没发现的错误。
...
$$
\begin{aligned}
Q(\varphi)&=\sum_{{(i,j)\in E_1 \atop (k,l)\in E_2}\atop {\{k\}=\varphi(i)\atop\{l\}=\varphi(j)}}\bar{c}((i,j)\to(k,l))&\text{(sub.)}\\
&+\sum_{{(i,j)\in E_1 \atop (k,l)\notin E_2}\atop {\{k\}=\varphi(i)\atop\{l\}=\varphi(j)}}c((i,j)\to\epsilon)&\text{(rem.)}\\
&+\sum_{{(i,j)\notin E_1 \atop (k,l)\in E_2}\atop {\{k\}=\varphi(i)\atop\{l\}=\varphi(j)}}c(\epsilon\to(k,l))&\text{(rem.)}
\end{aligned}\tag {11}
$$
