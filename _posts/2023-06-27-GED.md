...
$$
\begin{aligned}
Q(\varphi)&=\sum_{{(i,j)\in E_1, (k,l)\in E_2, \{k\}=\varphi(i), \{l\}=\varphi(j)}}\bar{c}((i,j)\to(k,l))&\text{(sub.)}\\
&+\sum_{{(i,j)\in E_1, (k,l)\notin E_2, \{k\}=\varphi(i), \{l\}=\varphi(j)}}c((i,j)\to\epsilon)&\text{(rem.)}\\
&+\sum_{{(i,j)\notin E_1, (k,l)\in E_2, \{k\}=\varphi(i), \{l\}=\varphi(j)}}c(\epsilon\to(k,l))&\text{(rem.)}
\end{aligned}\tag {11}
$$
